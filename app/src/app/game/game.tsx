...
//       onLeave();
  ...
  const thePlayer = gameState?.players.find((player) => player.id === id);

  return (
    <SafeAreaView>
      <View style={styles.topBarWrapper}>
        <View style={styles.playersWrapper}>
          {gameState?.players.map((player) => player.id !== thePlayer?.id && (
            <Player stringUnderAvatar={player.hand.length.toString()} active={player.id === gameState?.players?.[gameState?.currentPlayer]?.id} />
          ))}
        </View>
        <View>
          <Image source={deckImage} style={styles.deckImage} />
          <Text>
            {gameState?.deck.length.toString()}
          </Text>
        </View>
      </View>
      <View>
        <Text>
          {gameState?.players[gameState?.currentPlayer]?.id === id && 'Your turn'}
        </Text>
      </View>
      <View style={styles.pile}>
        {gameState?.discardedPile.map(((card, index) => (
          <View style={[
      ...
      <View>
        ...
        <View>
          <Player stringUnderAvatar='You' active={id === gameState?.players?.[gameState?.currentPlayer]?.id} />
        </View>
        <View>
